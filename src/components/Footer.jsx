    ...
    <footer className="relative overflow-hidden border-t border-line bg-bg-2 pt-20">
      ...
      <div className="relative mx-auto max-w-[1600px] px-5 sm:px-10 lg:px-16">
        ...
        <motion.div
          ...
          className="relative mb-20 overflow-hidden rounded-2xl border border-violet/25 bg-surface p-8 sm:p-14"
        >
          <div className="absolute inset-0 bg-grid bg-grid-fade opacity-40" />
          <div className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-violet/25 blur-[90px]" />
          <div className="relative flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
            <div>
              <p className="mb-3 font-tech text-xs uppercase tracking-[0.35em] text-violet">Final call</p>
              <h3 className="font-display uppercase leading-[0.95] tracking-tight text-[clamp(2rem,4.5vw,3.6rem)] text-ink">
                Your aim <span className="text-gradient-violet">deserves</span>
                <br />
                the best gear.
              </h3>
            </div>
            <button
              ...
              className="interactive group inline-flex shrink-0 items-center gap-2 rounded-full bg-violet px-8 py-4 font-tech text-sm font-semibold uppercase tracking-widest text-bg transition-colors hover:bg-violet-bright glow-violet"
            >
              Shop now
        ...
        <div className="grid grid-cols-2 gap-10 pb-16 md:grid-cols-3 lg:grid-cols-5">
          ...
          <div className="col-span-2 lg:col-span-2">
            <div className="mb-5 flex items-center gap-3">
              <div className="relative flex h-9 w-9 items-center justify-center">
                <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-violet-deep to-violet-bright blur-[6px]" />
            ...
            <div className="flex flex-col gap-2.5 font-body text-sm text-dim">
              <span className="flex items-center gap-2.5">
                <MapPin className="h-3.5 w-3.5 text-violet" /> Hyderabad, Telangana, India
              </span>
              <a href="mailto:hello@ryugear.in" className="flex items-center gap-2.5 transition-colors hover:text-violet-bright">
                <Mail className="h-3.5 w-3.5 text-violet" /> hello@ryugear.in
              </a>
              <span className="flex items-center gap-2.5">
                <Phone className="h-3.5 w-3.5 text-violet" /> +91 90000 00000
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2.5">
              {shop.map((item) => (
                <li key={item}>
                  <a href="#product-showcase" className="font-body text-sm text-mut transition-colors hover:text-violet-bright">
                    {item}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2.5">
              {support.map((item) => (
                <li key={item}>
                  <a href="#" className="font-body text-sm text-mut transition-colors hover:text-violet-bright">
                    {item}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2.5">
              {social.map(({ icon: Icon, label }) => (
                <li key={label}>
                  <a href="#" className="interactive flex items-center gap-2.5 font-body text-sm text-mut transition-colors hover:text-violet-bright">
                    <Icon className="h-3.5 w-3.5 text-violet" /> {label}
                  </a>
                </li>
              ))}
            </ul>
        ...
        {/* Bottom bar */}
        <div className="flex flex-col items-center justify-between gap-3 border-t border-line py-6 font-tech text-[10px] uppercase tracking-[0.25em] text-dim sm:flex-row">
